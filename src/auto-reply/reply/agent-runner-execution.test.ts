...
describe("runAgentTurnWithFallback", () => {
  ...
  it("suppresses visible pre-reply failures when requested", async () => {
    state.runEmbeddedPiAgentMock.mockRejectedValueOnce(
      new Error("INVALID_ARGUMENT: some other failure"),
    );
    ...
    expect(result.kind).toBe("final");
    if (result.kind === "final") {
      expect(result.payload.text).toBe("NO_REPLY");
    }
  });

  it("passes suppressVisibleErrorReplies through to the embedded run", async () => {
